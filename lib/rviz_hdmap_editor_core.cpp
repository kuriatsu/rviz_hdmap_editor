...
void RvizHdmapEditorCore::makeWhiteline()
{
	...
	std_msgs::ColorRGBA color{1.0, 1.0, 1.0, 1.0};
...
void RvizHdmapEditorCore::makeRoadedge()
{
	...
	std_msgs::ColorRGBA color{0.2, 0.2, 0.2, 1.0};
...
void RvizHdmapEditorCore::makeStopline()
{
	...
	std_msgs::ColorRGBA color{1.0, 0.0, 0.0, 1.0};
...
void RvizHdmapEditorCore::makeNode()
{
	...
	std_msgs::ColorRGBA color{0.0, 1.0, 0.0, 1.0};
...
{
	if (!hdmap_info_dict.at("railroad").is_visible) return;

	geometry_msgs::Point p;
    std::vector<std::string> added_points;
    auto &railroad_data = hdmap_info_dict.at("railroad").data;
    auto &area_data = hdmap_info_dict.at("area").data;
    auto &line_data = hdmap_info_dict.at("line").data;
    auto &point_data = hdmap_info_dict.at("point").data;

	std_msgs::ColorRGBA color{0.0, 1.0, 1.0, 1.0};
...
void RvizHdmapEditorCore::makeCrosswalk()
{
	...
	std_msgs::ColorRGBA color{1.0, 1.0, 0.0, 1.0};

	for (const auto &crosswalk_elem : crosswalk_data)
	{
		for (int line_id = std::stoi(area_data.at(crosswalk_elem.second[1])[1]); line_id <= stoi(area_data.at(railroad_elem.second[1])[2]); ++line_id)
		{
			for (int i=1; i<2; ++i)
			{
				std::string point_id = line_data.at(std::to_string(line_id))[i];
...
void RvizHdmapEditorCore::makeIntersection()
{
	...
	std_msgs::ColorRGBA color{0.0, 0.5, 0.5, 1.0};

	for (const auto &intersection_elem : intersection_data)
	{
		for (int line_id = std::stoi(area_data.at(intersection_elem.second[1])[1]); line_id <= stoi(area_data.at(railroad_elem.second[1])[2]); ++line_id)
		{
			for (int i=1; i<2; ++i)
			{
				std::string point_id = line_data.at(std::to_string(line_id))[i];
...
void RvizHdmapEditorCore::makePole()
{
	...
	std_msgs::ColorRGBA color{0.5, 0.5, 0.0, 1.0};

	for (const auto &pole_elem : pole_data)
	{

		std::string point_id = vector_data.at(pole_elem.second[1]);
        if (std::find(added_points.begin(), added_points.end(), point_id) != added_points.end())
        {
            continue;
        }
...
void RvizHdmapEditorCore::makeSignal()
{
	...
	std_msgs::ColorRGBA color{0.0, 0.5, 1.0, 1.0};

	for (const auto &signal_elem : signal_data)
	{

		std::string point_id = vector_data.at(signal_elem.second[1]);
        if (std::find(added_points.begin(), added_points.end(), point_id) != added_points.end())
        {
            continue;
        }
...
void RvizHdmapEditorCore::makeLane()
{
	...
	std_msgs::ColorRGBA color{0.5, 0.0, 0.5, 1.0};
